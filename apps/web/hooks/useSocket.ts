...
// import { SOCKET_EVENTS, type DrawingObject, type PresenceUser, type CursorPosition, type ChatMessage } from "@repo/websocket";
...
// //   useEffect(() => {
// //     if (!enabled) return; // solo mode — don't connect
...
// //     socket.on(SOCKET_EVENTS.PRESENCE_SYNC, (users: PresenceUser[]) => setOnlineUsers(users));
// //     socket.on(SOCKET_EVENTS.PRESENCE_JOIN, (user: PresenceUser) => {
// //       addUser(user);
// //       toast(`${user.name} joined the board`);
// //     });
// //     socket.on(SOCKET_EVENTS.PRESENCE_LEAVE, ({ userId, name }: { userId: string; name: string }) => {
// //       removeUser(userId);
// //       toast(`${name} left the board`);
// //     });

// //     socket.on(SOCKET_EVENTS.CURSOR_MOVE, (cursor: CursorPosition) => updateCursor(cursor));

// //     socket.on(SOCKET_EVENTS.DRAWING_CREATE, ({ object }: { object: DrawingObject }) => addObject(object));
// //     socket.on(SOCKET_EVENTS.DRAWING_UPDATE, ({ objectId, changes }: { objectId: string; changes: any }) =>
// //       updateObject(objectId, changes)
// //     );
// //     socket.on(SOCKET_EVENTS.DRAWING_DELETE, ({ objectId }: { objectId: string }) => removeObject(objectId));
// //     socket.on(SOCKET_EVENTS.BOARD_CLEARED, () => clearObjects());

// //     socket.on(SOCKET_EVENTS.CHAT_MESSAGE, (msg: ChatMessage) => addMessage(msg));
...
// //   const sendDrawingCreate = (object: Omit<DrawingObject, "id" | "authorId">) => {
// //     socketRef.current?.emit(SOCKET_EVENTS.DRAWING_CREATE, { boardId, object });
// //   };

// //   const sendDrawingUpdate = (objectId: string, changes: any) => {
// //     socketRef.current?.emit(SOCKET_EVENTS.DRAWING_UPDATE, { boardId, objectId, changes });
// //   };

// //   const sendDrawingDelete = (objectId: string) => {
...
//   const sendDrawingCreate = (
//     object: Omit<DrawingObject, "id" | "authorId">
//   ) => {
//     socketRef.current?.emit(SOCKET_EVENTS.DRAWING_CREATE, {
//       boardId,
//       object,
//     });
//   };
...
//   const sendDrawingDelete = (objectId: string) => {
//     socketRef.current?.emit(SOCKET_EVENTS.DRAWING_DELETE, {
//       boardId,
//       objectId,
//     });
//   };
...
//   const sendClearBoard = () => {
//     socketRef.current?.emit(SOCKET_EVENTS.BOARD_CLEAR, {
//       boardId,
//     });
//   };
